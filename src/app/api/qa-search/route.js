...
export async function POST(req) {
  ...
  if (lang === "en" && !isKeywordQuery && !hasValidIntent(query)) {
    console.timeEnd("TOTAL");
  ...
  const semanticGap = secondBest
    ? best.confidenceScore - secondBest.confidenceScore
    : 0;
  ...
  if (
    ...
    const fuzzyMatchItem = items.find((item) => {
      ...
      return enScore >= 0.7 || knScore >= 0.7;
    });
  ...
  for (const item of scored) {
    ...
    const qText = lang === "kn" ? item.question_kn : item.question_en;
  ...
  console.timeEnd("TOTAL");
  const bestDoc = await adminDB.collection("qna_items").doc(best.id).get();
  ...
  return NextResponse.json({
    ...
    bestMatch: {
      id: best.id,
      question: lang === "kn" ? bestData.question_kn : bestData.question_en,
      answer: lang === "kn" ? bestData.answer_kn : bestData.answer_en,
